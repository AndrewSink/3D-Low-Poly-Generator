...
exports.getImportGetTargetCode = getImportGetTargetCode;
...
function getImportGetTargetCode(esModule, loaderContext, insertIsFunction) {
  const modulePath = stringifyRequest(loaderContext, `!${_path.default.join(__dirname, "runtime/getTarget.js")}`);
  return esModule ? `${!insertIsFunction ? `import getTarget from ${modulePath};` : ""}` : `${!insertIsFunction ? `var getTarget = require(${modulePath});` : ""}`;
}
